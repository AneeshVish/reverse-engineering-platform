"""Desktop-sdk tests: DesktopClient/DesktopManager get_investigation."""
...
def test_client_get_investigation() -> None:
    ...
    investigation = client.get_investigation(submission.job_id)
    assert investigation.id
    assert isinstance(investigation.findings, list)


def test_manager_get_investigation(tmp_path: Path) -> None:
    ...
    investigation = manager.get_investigation(submission.job_id)
