...
class PluginManager:
    ...
    def load_plugins(self):
        ...
        if not self.plugin_directory.exists():
            self.logger.warning(f"Plugin directory not found: {self.plugin_directory}")
            return
    ...
    def load_plugin(self, plugin_file):
        ...
        try:
            ...
            if not hasattr(module, 'register_plugin'):
                self.logger.warning(f"Plugin {plugin_name} does not have a register_plugin function")
                return False
            
            # Register the plugin
            plugin = module.register_plugin()
            if not plugin:
                self.logger.warning(f"Plugin {plugin_name} registration failed")
                return False
    ...
    def _register_hooks(self, plugin):
        ...
        for hook_name in self.hooks.keys():
            if hasattr(plugin, hook_name):
                ...
                self.logger.debug(f"Registered hook {hook_name} for plugin {plugin.__class__.__name__}")
    ...
    def execute_hook(self, hook_name, *args, **kwargs):
        ...
        for plugin in self.hooks[hook_name]:
            try:
                ...
                self.logger.error(f"Error executing hook {hook_name} for plugin {plugin.__class__.__name__}: {e}")
