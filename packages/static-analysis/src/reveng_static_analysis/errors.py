"""Static-analysis error taxonomy over the shared engineering error types.
...
class StaticError(Exception):
    """Base exception for static-analysis failures."""

    code: str = "STATIC.ERROR"
...
class RegistrationError(StaticError):
    code = "STATIC.REGISTRATION"


class AnalysisError(StaticError):
    code = "STATIC.ANALYSIS"


class ParserError(StaticError):
    code = "STATIC.PARSER"


class ExtractionError(StaticError):
    code = "STATIC.EXTRACTION"


class ValidationError(StaticError):
    code = "STATIC.VALIDATION"


def make_error(code: str, message: str, **context: Any) -> EngError:
    """Construct an ``EngError`` with a static-analysis-namespaced code."""
...
def guard(fn: Callable[[], T]) -> Result[T]:
    ...
    ``StaticError`` maps to its declared code; any other exception maps to
    ``STATIC.UNEXPECTED``. This keeps raw exceptions off the public API.
    """
    ...
    try:
        ...
        return Result.failure(
            err("STATIC.UNEXPECTED", str(exc), exception_type=type(exc).__name__)
        )
