"""Immutable, key-sorted graph properties.

A ``PropertyBag`` is an immutable mapping from string keys to scalar values.
Iteration and serialization are always key-sorted so equal properties produce
...
class PropertyBag:
    ...
    def with_value(self, key: str, value: PropertyValue) -> PropertyBag:
        merged = {k: v for k, v in self._items}
        merged[key] = value
        return PropertyBag.of(merged)
